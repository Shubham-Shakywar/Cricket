...
const ReportProblem = (props : reportProps) => {
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      ...
      const response = await fetch("https://cricketwicket.biz/api/v1/reports/submit", requestOptions);
      const result = await response.json();

      console.log("submit report response", result);

      if (result.message === "Report submitted successfully") {
        navigation.navigate("ReportLists");
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.contentContainer}>
        <TouchableOpacity style={styles.btn1} onPress={() => navigation.navigate("ReportLists")}>
          <Text style={styles.btnTxt}>Report List</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.cont} onPress={openGallery}>
          <Image
            source={require('../../assets/icons8-add-picture-50.png')}
            style={styles.addPic}
          />
